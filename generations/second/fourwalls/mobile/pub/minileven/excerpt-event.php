...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<?php if ( '1' == get_option( 'wp_mobile_excerpt' ) ) : ?>
			<?php $datestring_from = get_post_meta(get_the_ID(), 'event_date_from', true); ?>
			<?php $datestring_to = get_post_meta(get_the_ID(), 'event_date_to', true); ?>
			<?php if (strlen($datestring_from) > 1) : ?>
				<a href="<?php the_permalink(); ?>" rel="bookmark"><span>
			<?php if ($datestring_from == $datestring_to) : ?>
				<?php echo date("jS F", $datestring_from); ?>
			<?php else: ?>
				<?php if (date("F", $datestring_from) == date("F", $datestring_to)) : ?>
					<?php echo date("jS", $datestring_from) . ' - ' . date("jS F", $datestring_to); ?>
				<?php else: ?>
					<?php echo date("jS F", $datestring_from) . ' - ' . date("jS F", $datestring_to); ?>
				<?php endif; ?>
			<?php endif; ?>
				</span></a>
			<?php endif; ?>
